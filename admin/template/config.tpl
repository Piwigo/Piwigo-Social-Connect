...
<fieldset id="providers">
  ...
  <legend>{'Providers'|@translate}</legend>
  
{foreach from=$PROVIDERS item=provider key=p}
<div data-p="{$p}" class="provider {if $CONFIG[$p].enabled}enabled{else}disabled{/if}">
  <h4>{$provider.provider_name}</h4>
  
  <table><tr>
    <td>
      <img src="{$OAUTH_PATH}template/icons/{$p|strtolower}_big.png">
    </td>
